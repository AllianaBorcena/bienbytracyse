    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function($query){
                ...
                <a class="dropdown-item" href="'.route('admin.product-icing.show-index', $query->id).'">Product Variants</a>
                </div>
            </div>';
            ...
            ->addColumn('image', function($query){
                return '<img width="60px" src="'.asset($query->thumb_img).'">';
            })
            ->rawColumns(['price','status','show_at_home','action','image'])
            ->setRowId('id');
    }
